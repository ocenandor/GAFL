...
def run_epoch(model, iterator,
    ...
    predictions_all = torch.tensor([], device=device)
    targets_all = torch.tensor([], device=device)
    ...
    with torch.set_grad_enabled(is_train):
        for (images, targets) in tqdm(iterator, desc=f"{phase}", ascii=True):
            ...
            predictions_all = torch.cat((predictions_all, predictions.detach()), 0)
            targets_all = torch.cat((targets_all, targets.detach()), 0)
        ...
        for metric_name in metrics.keys():
                epoch_metrics[metric_name] += metrics[metric_name](predictions_all.detach(), targets_all)
